...
def receiveOnePing(mySocket, ID, timeout, destAddr):
    ...
    while 1:
        ...
        hType, code, checkSum, hID, sequence = struct.unpack("bbHHh", header)
        if hID == ID:
            bytes = struct.calcsize("d")
            startTime = struct.unpack("d", recPacket[28:28 + bytes])[0]
            timeDiff = timeReceived-startTime
            return(timeDiff)
...
def ping(host, timeout=1):
    ...
    stdev = (sum([(delay-packet_avg)**2 for delay in delays])/len(delays))**0.5
